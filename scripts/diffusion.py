...
def get_time_schedule(args, device):
    n_timestep = args.diffusion_steps
    eps_small = 1e-3
...
def sample_from_model_test(coefficients, model, n_time, x_init, T, real_data, opt):
    x = x_init
    with torch.no_grad():
        for i in reversed(range(n_time)):
            t = torch.full((x.size(0),), i, dtype=torch.int64).to(x.device)
            t_time = t
            x_cat = torch.cat((x, real_data), dim=1)
            x_0 = model(x_cat, t_time)
            x_new = sample_posterior(coefficients, x_0, x, t)
            x = x_new.detach()
