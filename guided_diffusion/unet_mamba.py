...
from .mamba_block import Mamba3DBlock
from .nn import (
    checkpoint,
    conv_nd,
    linear,
...
class WaveletGatingDownsample(nn.Module):
    ...
    def forward(self, x, temb):
        # Get gating values
        p = (
            self.pooling(x).squeeze(-1).squeeze(-1).squeeze(-1)
        )  # Average pool over feature dimension
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.use_wgupdown = use_wgupdown
        ...
        if up:
            ...
            # When using wavelet gated upsampling
            if self.use_wgupdown:
                self.h_upd = WaveletGatingUpsample(channels, temb_dim)
                self.x_upd = WaveletGatingUpsample(channels, temb_dim)
            ...
            # when using wavelet gated downsampling
            if self.use_wgupdown:
                self.h_upd = WaveletGatingDownsample(channels, temb_dim)
                self.x_upd = WaveletGatingDownsample(channels, temb_dim)
        else:
...
class UNetModel_mamba(nn.Module):
    """
    ...
    def __init__(
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            for i in range(num_res_blocks + 1):
                ...
                if ch != ich:
                    print(
                        f" channels don't match: {ch=: >4}, {ich=: >4}, {level=}, {i=}"
                    )
                else:
                    print(
                        f" channels do    match: {ch=: >4}, {ich=: >4}, {level=}, {i=}"
                    )
                layers = [
                    ResBlock(
    ...
    def to(self, *args, **kwargs):
        ...
        if isinstance(args[0], (list, tuple)) and len(args[0]) > 1:
            ...
            ):  # if self.devices has not been set yet, read it from params
    ...
    def forward(self, x, timesteps, y=None):
        ...
        assert (
            x.device == self.devices[0]
        ), f"{x.device=} does not match {self.devices[0]=}"
        assert (
            timesteps.device == self.devices[0]
        ), f"{timesteps.device=} does not match {self.devices[0]=}"
...
class SuperResModel(UNetModel_mamba):
    """
